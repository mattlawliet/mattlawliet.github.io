...
def write_pages() -> int:
    """Give every project a real URL at /p/<id>/ so a pasted link carries its own card.

    The page is index.html with the block between the og markers swapped for that
    project's tags. It is not a redirect and not a separate template: same markup, same
    scripts, so there is only ever one page to maintain. main.js reads the project id
    back out of the path.

    The images these point at are rendered by tools/make-cards.sh, which needs a browser
    and so cannot run from here. Re-run it when a name, download count or model changes.
    """
    html = ROOT / "index.html"
    if not html.is_file():
        return 0
    src = html.read_text()
    if "<!-- og:start -->" not in src:
        print("index.html has no og markers; skipping project pages")
        return 0
    ...
    for p in doc["projects"]:
        pid = p["id"]
        desc = p.get("blurb") or ""
        tags = f"""<!-- og:start --><!-- generated by sync.py; edit index.html, not this -->
  <link rel="canonical" href="{SITE}/p/{pid}/" />
  <meta property="og:type" content="website" />
  <meta property="og:site_name" content="Matt.dev" />
  <meta property="og:url" content="{SITE}/p/{pid}/" />
  <meta property="og:title" content="{esc(p["name"])} — Matt.dev" />
  <meta property="og:description" content="{esc(desc)}" />
  <meta property="og:image" content="{SITE}/og/{pid}.png" />
  <meta property="og:image:width" content="1200" />
  <meta property="og:image:height" content="630" />
  <meta name="twitter:card" content="summary_large_image" />
  <!-- og:end -->"""
        out = re.sub(r"<!-- og:start -->.*?<!-- og:end -->", lambda _: tags, src, flags=re.S)
        out = out.replace("<title>Matt.dev</title>", f"<title>{esc(p['name'])} — Matt.dev</title>", 1)
        page = ROOT / "p" / pid / "index.html"
        page.parent.mkdir(parents=True, exist_ok=True)
        if not page.is_file() or page.read_text() != out:
            page.write_text(out)
            written += 1
    if written:
        print(f"pages: {written} project page(s) written under p/")
    return written
...
def main() -> int:
    ...
    if not total:
        print("\nno changes")
        # index.html can change without projects.json doing so, and the pages are
        # copies of it, so they are rebuilt regardless of whether the data drifted
        return 2 if (not args.check and write_pages()) else 0
